            ...
            for (std::size_t index = 0; index < subgrid_array.size(); ++index) {
                if (subgrid_array[index] != 0) {
                    ...
                    if (b == 0 && index == 41020) {
                        ...
                        assert(node_values_index[1] == 0.030521584007828874); // PyAPI: `subgrid.node_values[1][20]`
                        assert(node_values_index[2] == 0.030521584007828874); // PyAPI: `subgrid.node_values[2][20]`

                        // PyAPI: `grid.subgrid(0, 0, 0).to_array(subgrid.shape)[16][20][20]`
                        assert(subgrid_array[index] == -4.9361569250960126e-07);
                    }
                    break;
                }
            }
