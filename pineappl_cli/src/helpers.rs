...
pub fn convolute(
    ...
    pdf_pdg_id = pdf.set().entry("Particle").unwrap().parse::<i32>().unwrap();

    let pdf = |id, x, q2| pdf.xfx_q2(id, x, q2);
    let no_pdf = |_, x, _| x;

    let xfx1: Box<dyn Fn(i32, f64, f64) -> f64> = if match initial_state_1 {
        2212 => Box::new(&proton_pdf),
